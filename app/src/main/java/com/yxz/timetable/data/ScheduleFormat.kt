...
object ScheduleFormat {
     ...
     * App 内「格式说明」页显示的文字。
     *
     * ## 为什么它在这里，而不是在设置页那个 Fragment 里
     *
     * 它原本写在 `SettingsFragment` 的伴生对象里，结果是**唯一没人管的地方** ——
     * 格式从 v1 一路升到 v3，它一直写着「当前是 1」，
     * 而没有任何测试能发现（Fragment 依赖 Android 框架，JVM 单元测试加载不了）。
     *
     * 挪到这里有两个好处：
     *   ① 紧挨着解析和生成代码，改格式时顺手就能看见
     *   ② **可以被单元测试断言**（见 FormatTextTest）
     *
     * 版本号用 `$VERSION` 插值，不写死数字。
     *
     * 通用教训：**凡是「用户会照着做」的文本，都值得放在能被测试的地方。**
     * 它离代码越远，脱节得越久才被发现。
     */
    fun formatHelp(): String = """
        【顶层】
        format    固定为 "$FORMAT_ID"
        version   格式版本，当前是 $VERSION
        term      学期设置（可选）
        dayTypes  启用哪些日型（可选）
        courses   课程数组（可选）
        templates 作息模板（可选）
