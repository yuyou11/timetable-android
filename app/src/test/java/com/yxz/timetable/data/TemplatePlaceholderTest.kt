...
/**
 * 守住内置模板的占位格：**时刻要对齐内置作息表 [Slots]，且该有的都要有。**
 *
 * ============================================================
 *  这个文件的历史（读之前需要知道）
 * ============================================================
 *
 * 它最初是为另一个问题写的：用户那份模板的占位格时刻和 `Slots` 差了 30 分钟，
 * 而当时的引擎靠 `it.start == Slots.start(...)` 找占位格，对不上就顶不掉，
 * 结果是满屏「空档 · 机动」。
 *
 * **那个机制现在已经不存在了。** 节次时间改成从模板的占位格里读
 * （见 [TemplateSet.slotStart]），两边同源，天然对得上。
 *
 * 所以这个文件现在的职责变小了，但没消失：
 *
 *   1. **内置模板和内置作息表必须一致** —— 不然「什么都不改」的用户
 *      看到的就是一套混乱的时间
 *   2. **该有的占位格不能少** —— 缺一段的后果见下面那条测试
 *   3. **整学期不该冒出「空档 · 机动」** —— 不管机制怎么变，
 *      这条端到端的判据都成立，而且是用户最容易看见的
 *
 * 用户自定义的模板**不再受「必须等于 Slots」约束**，
 * 那是 [SlotTimeTest] 的领域。
 */
